...
async function updateOrder(status) {
    ...
    try {
        ...
        const response = await fetch('http://localhost:5000/update-refillrequest-status', {
            method: 'PUT',
        ...
        if (response.ok) {
            ...
            alert('Refill Request status updated successfully');
            window.location.href = 'refillRequest.html';
        } else {
            ...
            alert(`Error updating refill request status: ${errorMessage}`);
        }
    } catch (error) {
        // Catch and log any network or other errors
        console.error('Error:', error);
        alert('An error occurred while updating the refill request status.');
    }
}